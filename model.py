...
class DRRN(torch.nn.Module):
    ...
    def state_action_attention(self, state_batch, act_batch):
        ...
        obs_hash_out = torch.repeat_interleave(obs_hash_out, torch.tensor(act_sizes, dtype=torch.long, device=device), dim=0)
        look_hash_out = self.packed_hash(state.description, mlp=False)
        look_hash_out = torch.repeat_interleave(look_hash_out, torch.tensor(act_sizes, dtype=torch.long, device=device), dim=0)
        inv_hash_out = self.packed_hash(state.inventory, mlp=False)
        inv_hash_out = torch.repeat_interleave(inv_hash_out, torch.tensor(act_sizes, dtype=torch.long, device=device), dim=0)
